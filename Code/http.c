...
//     /* Decode URL escape sequences in the value safely */
//     char temp_value[512];  // Temporary buffer to ensure no overflow
//     url_decode(temp_value, sp);
//     temp_value[511] = '\0';  // Ensure null termination

//     strncpy(value, temp_value, 511);
//     value[511] = '\0';
...
//     if (buf_len >= 512)  // Prevent buffer overflow
...
//     /* Decode URL escape sequences in the value safely */
//     char temp_value[512];  // Temporary buffer to ensure no overflow
//     strncpy(temp_value, sp, 511);  // Ensure sp is copied safely
//     temp_value[511] = '\0';  // Null terminate

//     url_decode(value, temp_value);
//     value[511] = '\0';  // Ensure null termination

//     /* Ensure envvar is not too long */
//     if (strlen(buf) >= 500)  // Leaving space for "HTTP_"
//         return "Header name too long (5)";

//     if (strcmp(buf, "CONTENT_TYPE") != 0 && strcmp(buf, "CONTENT_LENGTH") != 0) {
//         snprintf(envvar, 512, "HTTP_%s", buf);
//     } else {
//         strncpy(envvar, buf, 511);
//         envvar[511] = '\0';
...
//     /* Ensure that envvar and value are valid before calling setenv */
//     if (strlen(envvar) > 500 || strlen(value) > 500)
//         return "Environment variable too long (6)";
...
//     /* Decode URL escape sequences in the value safely */
//     char temp_value[512];  
//     strncpy(temp_value, sp, 511);  
//     temp_value[511] = '\0';

//     url_decode(value, temp_value);  // Ensure safe decoding
//     value[511] = '\0';  
...
//     /* Check if envvar or value is valid before setting env */
//     if (strlen(envvar) > 500 || strlen(value) > 500)
//         return "Environment variable too long (6)";

//     if (setenv(envvar, value, 1) < 0) {  // Handle failure
//         return "Failed to set environment variable (7)";
//     }
...
//     /* Ensure the value does not exceed the buffer size */
//     if (strlen(value) >= 512) {
...
void http_err(int fd, int code, char *fmt, ...) {
    if (fd < 0) {  
        warnx("http_err: Invalid file descriptor %d", fd);
        return;
    }

    if (write(fd, "", 0) < 0) {
        warnx("http_err: Socket is closed, skipping response");
        close(fd);
        return;
    }

    if (write(fd, "HTTP/1.0 ", 9) < 0) {
        warnx("http_err: write failed, socket may be closed");
        close(fd);
        return;
    }
